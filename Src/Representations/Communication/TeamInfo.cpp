...
void TeamInfo::read(In& stream)
{
  ...
  STREAM(fieldPlayerColor); // TEAM_BLUE, TEAM_RED, TEAM_YELLOW, TEAM_BLACK, ...
  STREAM(goalkeeperColor); // TEAM_BLUE, TEAM_RED, TEAM_YELLOW, TEAM_BLACK, ...
...
void TeamInfo::draw() const
{
  DECLARE_DEBUG_DRAWING3D("representation:TeamInfo", "field");
  {
    ...
    drawDigit(score / 10, Vector3f(x, yPosLeftSideline, 1000), 200, fieldPlayerColor);
    drawDigit(score % 10, Vector3f(x + 270, yPosLeftSideline, 1000), 200, fieldPlayerColor);
  };
}

OwnTeamInfo::OwnTeamInfo()
{
  fieldPlayerColor = Global::settingsExist() ? Global::getSettings().fieldPlayerColor : TEAM_GREEN;
  goalkeeperColor = Global::settingsExist() ? Global::getSettings().goalkeeperColor : TEAM_WHITE;
}
...
void OwnTeamInfo::draw() const
{
  ...
  DEBUG_DRAWING("representation:OwnTeamInfo", "drawingOnField")
  {
    ...
    DRAW_TEXT("representation:OwnTeamInfo", xPosOwnFieldBorder + 200, yPosRightFieldBorder - 100,
              (xPosOwnFieldBorder / -5200.f) * 140, ColorRGBA::red,
              "Team color: " << TypeRegistry::getEnumName((Settings::TeamColor)fieldPlayerColor));
  }
}

OpponentTeamInfo::OpponentTeamInfo()
{
  fieldPlayerColor = 1 ^ (Global::settingsExist() ? Global::getSettings().fieldPlayerColor : TEAM_RED);
  goalkeeperColor = 1 ^ (Global::settingsExist() ? Global::getSettings().fieldPlayerColor : TEAM_BLUE);
}
