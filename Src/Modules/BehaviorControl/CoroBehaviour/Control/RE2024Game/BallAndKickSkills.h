...
// FIXME - extract common elements of pass and shoot tasks using a bitmask
// of permitted kicks. For now we have two different tasks that are very
...
namespace RE2024
{
  ...
  private:
    ...
    {
      kickDistance = target.norm() + 500.f; // approximation wtih margin //used to be 5000
      kickAngle = target.angle();

      // choose the kick type (based on distance and angle)
      // Add some Hysteresis so quicker
      ...
      kickAlignPrecisely = true; // default unless changed below // TODO should it be false?
      ...
      {//Default back to short kick
        kickType = !targetIsLeft ? KickInfo::walkForwardsLeft : KickInfo::walkForwardsRight;
      }

      ACTGRAPH_FMT("kickAngle = {}_deg", toDegrees(kickAngle));
      ACTGRAPH_FMT("kicDistance = {}", kickDistance);
