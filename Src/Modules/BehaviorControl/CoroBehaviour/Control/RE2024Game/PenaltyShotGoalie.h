...
namespace RE2024
{
  ...
  CRBEHAVIOUR(PenaltyShotGoalieReadyTask)
  {
    ...
    void operator()(void)
    {
      // position goalie forward of the goal line so that the back of its foot just covers the full line width
      Pose2f goaliePose = theRobotPose.toRobotCoordinates(Pose2f(0,
                                                                 theFieldDimensions.xPosOwnGroundLine +
                                                                     theRobotDimensions.soleToBackEdgeLength -
  ...
  {  
    ...
    void operator()(void)
    {
      ...
      while (!goalieSaveTask.isSuccess())
      {
        ...
        goalieSaveTask(params.enabledSaves, /* isSitting */ false, params.getUpAfterSaveDelayMs);
        CR_YIELD();
      }
