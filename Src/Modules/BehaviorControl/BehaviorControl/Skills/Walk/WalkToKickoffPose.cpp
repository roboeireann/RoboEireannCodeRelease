...
{,
  ...
  {,
    ...
    (float)(0.85f) walkInSpeed, ///< default speed to use during a walk in from the side
    (float)(0.85f) adjustingSpeed, ///< default speed to use while (coarse) adjusting position
...
class WalkToKickoffPoseImpl : public WalkToKickoffPoseImplBase
{
  option(WalkToKickoffPose)
  {
    ...
    const int timeUntilSetState = std::min<int>(theReceivedGameControlData.state == STATE_READY ? theGameInfo.secondaryTime * 1000 : 45000, 45000 - theExtendedGameInfo.timeSinceReadyStarted);
        ...
        if(targetRelative.translation.squaredNorm() < sqr(200.f))
        {
          if(theRobotPose.translation.squaredNorm() < sqr(theFieldDimensions.centerCircleRadius))
            ...
            goto adjustToFinalPose;
                ...
                (theGameInfo.isOurKick() || theRobotPose.translation.squaredNorm() > sqr(theFieldDimensions.centerCircleRadius + 200.f)))
          goto emergencyAdjustment;
      }
      action
      {
        theLookActiveSkill(false, /* ignoreBall: */ true);
        theWalkToPointSkill(targetRelative, theExtendedGameInfo.walkingInFromSidelines ? walkInSpeed : 1.f,
                            /* rough: */ targetRelative.translation.norm() < 500.f && timeUntilSetState < 10000, false, false, true);
      }
    }
    ...
    state(adjustToFinalPose)
    {
      ...
      {
        theLookActiveSkill(false, /* ignoreBall: */ true);
        theWalkToPointSkill(targetRelative, adjustingSpeed, /* rough: */ true, /* disableObstacleAvoidance: */ theTeamBehaviorStatus.role.isGoalkeeper(), false, true);
      }
    }
    ...
    state(adjustToFinalPosePrecisely)
    {
      ...
      {
        theLookActiveSkill(false, /* ignoreBall: */ true);
        theWalkToPointSkill(targetRelative, fineAdjustingSpeed, /* rough: */ true, false, false, true);
      }
    }
    ...
    state(stand)
    {
      transition
      {
        if(targetRelative.translation.squaredNorm() > sqr(300.f) || std::abs(targetRelative.rotation) > 15_deg ||
           ...
           (!theGameInfo.isOurKick() && theRobotPose.translation.squaredNorm() < sqr(theFieldDimensions.centerCircleRadius + 170.f)))
