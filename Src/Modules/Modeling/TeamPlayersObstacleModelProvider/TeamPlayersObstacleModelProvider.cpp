/**
 * @file TeamPlayersObstacleModelProvider.cpp
 *
...
#include "TeamPlayersObstacleModelProvider.h"
...
MAKE_MODULE(TeamPlayersObstacleModelProvider, modeling);

TeamPlayersObstacleModelProvider::TeamPlayersObstacleModelProvider()
{
  goalPosts.reserve(4);
...
void TeamPlayersObstacleModelProvider::update(TeamPlayersObstacleModel& teamPlayersObstacleModel)
{
  auto& obstacles = teamPlayersObstacleModel.obstacles;
  DECLARE_DEBUG_DRAWING("module:TeamPlayersObstacleModelProvider:others", "drawingOnField");
  obstacles.clear();
  ownTeam.clear();

  if(theGameInfo.isPenalized() || !theGroundContactState.contact || theFallDownState.state != theFallDownState.upright)
    return;
  ...
  ownTeam[theGameInfo.playerNumber] = theRobotPose;
  ...
  for(const auto& obstacle : theObstacleModel.obstacles)
  {
    ...
    {
      if(isGoalPost(p) || isTeammate(p, squaredDistanceThreshold, theGameInfo.playerNumber))
        continue;
      ...
      if(isInsideOwnDetectionArea(p, theGameInfo.playerNumber, obstacle.lastSeen))
  ...
  STOPWATCH("TeamPlayersObstacleModelProvider:obstaclesByTeammates")
  {
    for(auto const& teammate : theTeamData.teammates)
    {
  ...
  STOPWATCH("TeamPlayersObstacleModelProvider:clusterThing")
  {
    //cluster the others
    ...
    for(const auto& other : others)
    {
      if(other.second > 0 && !isTeammate(other.first.center, squaredDistanceThreshold))
        ...
        CIRCLE("module:TeamPlayersObstacleModelProvider:others", other.first.center.x(), other.first.center.y(), 100, 40, Drawings::dottedPen, ColorRGBA::red, Drawings::noBrush, ColorRGBA::black);
    }
  ...
  {
    if(tm.first == theGameInfo.playerNumber)
      continue;
    obstacles.emplace_back(tm.second.covariance.topLeftCorner(2, 2), tm.second.translation, theFrameInfo.time);
  }
}

bool TeamPlayersObstacleModelProvider::isInsideOwnDetectionArea(const Vector2f& position, int robotNumber, int lastSeen) const
{
  float tmp;
  return isInsideOwnDetectionArea(position, robotNumber, tmp, lastSeen);
}

bool TeamPlayersObstacleModelProvider::isInsideOwnDetectionArea(const Vector2f& position, int robotNumber, float& distance, int lastSeen) const
{
...
bool TeamPlayersObstacleModelProvider::collideOtherDetectionArea(const Vector2f& position, int robotNumber, std::map<int, RobotPose>& ownTeam,
...
bool TeamPlayersObstacleModelProvider::isGoalPost(const Vector2f& position) const
{
...
bool TeamPlayersObstacleModelProvider::isTeammate(const Vector2f& position, const float radius, int ignoreRobotNumber) const
{
  for(const auto& tm : ownTeam)
  {
...
bool TeamPlayersObstacleModelProvider::isTeammate(const Vector2f& position, const float radius) const
{
  for(const auto& tm : ownTeam)
  {
    if((position - tm.second.translation).squaredNorm() < radius)
...
void TeamPlayersObstacleModelProvider::setType(Obstacle& one, const Obstacle& other) const
{
