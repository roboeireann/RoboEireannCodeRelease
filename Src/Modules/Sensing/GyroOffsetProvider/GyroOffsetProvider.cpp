...
void GyroOffsetProvider::update(GyroOffset& gyroOffset)
{
  ...
  switch(state)
  {
    ...
    case set:
    {
      ...
      if (gyroDeviationX[0] < thresholdGyroDeviation && gyroDeviationY[0] < thresholdGyroDeviation &&
          gyroDeviationZ[0] < thresholdGyroDeviation // last 333ms
          && gyroDeviationX[2] < thresholdGyroDeviation && gyroDeviationY[2] < thresholdGyroDeviation &&
          gyroDeviationZ[2] < thresholdGyroDeviation) // last 666-999ms
      {
        ...
        {
          float maxOffset = std::max(
              std::max(std::abs(gyroOffset.offset.x().toDegrees()), std::abs(gyroOffset.offset.y().toDegrees())),
...
void GyroOffsetProvider::checkBodyDisconnection(GyroOffset& gyroOffset)
{
  if(lastGyroChange == 0 || lastGyros != theInertialData.gyro)
  {
    ...
    bool wasDisconnected = (gyroOffset.bodyDisconnect && lastGyroChange); 
  ...
  else if(theFrameInfo.getTimeSince(lastGyroChange) > maxGyroDelay && lastGyroChange > startTimestamp)
  {
    ...
    if (SystemCall::getMode() == SystemCall::physicalRobot &&
        theFrameInfo.getTimeSince(gyroStuckSoundTimestamp) > bodyDisconnectWaitTime)
    {
