...
DECL_TLOGGER(tlogger, "KickEngine", TextLogging::INFO);
...
KickEngine::KickEngine()
{
  ...
  while(file != nullptr)
  {
    ...
    if(strstr(name, ".kmc"))
    {
      ...
      else
      {
        ...
        TLOGW(tlogger, "KickRequest is missing the id for {}", parameters.name);
      }
    }
    file = readdir(dir);
  }
  ...
  for(int i = 0; i < KickRequest::newKick; ++i)
  {
    ...
    if(id == -1)
    {
      ...
      TLOGW(tlogger, "The kick motion file for id {} is missing", TypeRegistry::getEnumName(static_cast<KickRequest::KickMotionID>(i)));
    }
  }
