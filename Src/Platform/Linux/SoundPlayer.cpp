...
{
#ifdef TARGET_ROBOT
  textToSpeechVolumeFactor = 1.4f; // Increase text to speech volume by this factor (1.5 seems to be too high, results in cracking noise)
#else
  textToSpeechVolumeFactor = 1.2f; // speech is needs to be slightly louder than other sounds
#endif
  textToSpeechDurationStretch = 1.3f; // make the robot speak a bit slower
  textToSpeechPitch = 210.f; // Make the robot speak at a higher pitch
  textToSpeechPitchStddev = 40.f; // Make the robot more expressive

  flite_init();
  voice = register_cmu_us_slt(nullptr);

  flite_feat_set_float(voice->features, "duration_stretch", textToSpeechDurationStretch);
  flite_feat_set_float(voice->features, "int_f0_target_mean", textToSpeechPitch);
  flite_feat_set_float(voice->features, "int_f0_target_stddev", textToSpeechPitchStddev);
  ...
  for(;;)
  {
    ...
    if(first.isTextToSpeech && !first.fileOrText.empty())
    {
      ...
      flite_feat_set_float(soundPlayer.voice->features, "duration_stretch",
                           soundPlayer.textToSpeechDurationStretch * first.ttsDurationStretchFactor);
