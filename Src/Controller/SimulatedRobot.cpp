  ...
  robot(robot)
{
  ...
  robotNumber = getSceneNumber(robot);
  ...
  for(unsigned currentRobot = 0, count = RoboCupCtrl::application->getObjectChildCount(*group); currentRobot < count; ++currentRobot)
  {
    ...
    const int number = getSceneNumber(robot);
    if(number != robotNumber)
    {
      if (RobotNumber::isFirstTeamFromScene(number))
...
void SimulatedRobot::getWorldState(GroundTruthWorldState& worldState) const
{
  ...
  for(unsigned int i = 0; i < firstTeamRobots.size(); ++i)
  {
    ...
    newGTPlayer.number = RobotNumber::getPlayerNumberFromScene(getSceneNumber(firstTeamRobots[i]));
  ...
  {
    GroundTruthWorldState::GroundTruthPlayer newGTPlayer;
    newGTPlayer.number = RobotNumber::getPlayerNumberFromScene(getSceneNumber(secondTeamRobots[i]));
...
{
  return RobotNumber::isFirstTeamFromScene(getSceneNumber(obj));
}

int SimulatedRobot::getSceneNumber(const SimRobot::Object* obj)
{
  QString robotNumberString = obj->getFullName();
  int pos = robotNumberString.lastIndexOf('.');
  robotNumberString.remove(0, pos + 6);
  int sceneNumber = robotNumberString.toInt();

  RobotNumber::validateSceneNumber(sceneNumber);
