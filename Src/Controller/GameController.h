...
private:
  ...
  class TeamMessageCounter
  {
  public:
    int port = 0;
    // message count and invalid count for this cycle
    int messageCount = 0;
    int invalidCount = 0;

    void startLocal(int portIn);
    void receive();
  ...
  DECLARE_SYNC;
  static const int numPlayersPerTeam = RobotNumber::numPlayersPerTeam; // players + substitutes
  static const int numOfRobots = RobotNumber::numRobots; // 2 teams
  static const int numOfFieldPlayers = RobotNumber::numFieldPlayersPerTeam; // Keeper, Substitute
  ...
  static const int initialMessageBudget = halfTime * 2; // on avg 1 msg/sec for entire team
...
public:
   ...
   * @return true is gameInfo was actually written - it's not written on every cycle
   */
   ...
   */
  void setTeamInfos(Settings::TeamColor fieldPlayerColor1, Settings::TeamColor goalkeeperColor1,
...
private:
   ...
   */
  bool handleGlobalCommand(const std::string& command, bool fromConsole = false);
   ...
   * @param command The second part of the command (without "gc").
   */
  bool handleStateCommand(const std::string& command, bool fromConsole = false);

  /**
   * Handles commands that count goals.
   * @param command The second part of the command (without "gc").
   */
  bool handleGoalCommand(const std::string& command, bool fromConsole = false);

  /**
   * Handles commands that give free kicks.
   * @param command The second part of the command (without "gc").
   */
  bool handleSetPlayCommand(const std::string& command, bool fromConsole = false);
  ...
  /**
   * Handles commands for game phase
   * @param command The second part of the command (without "gc").
   */
  bool handleGamePhaseCommand(const std::string& command);
