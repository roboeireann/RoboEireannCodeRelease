...
DECL_TLOGGER(tlogger, "LogExtractor", TextLogging::INFO);
...
bool LogExtractor::saveImages(const std::string& path, const bool raw, const bool onlyPlaying, const int takeEachNthFrame = 1)
{
  ...
  TLOGI(tlogger, "takeEachNthFrame = {}", takeEachNthFrame);
  ...
  {
    logFrameNum++;
    TLOGD(tlogger, "frame {} [{}], jpegTimestamp {}", logFrameNum,
    ...
    {
      TLOGD(tlogger, "  -- no jpeg image");
      // theCameraImage.timestamp = 0;
      return true;
    }
    ...
    if(imageToExport)
    {
      ...
      // Open PNG file
      // if (theCameraInfo.camera == CameraInfo::upper) // FIXME: RV hack upper images only
      {
        ...
        if (theCameraInfo.camera == CameraInfo::upper)
        {
          unsigned delta = (prevTimestamp == 0) ? 9999 : imageToExport->timestamp - prevTimestamp;
          prevTimestamp = imageToExport->timestamp;
          TLOGI(tlogger, "  -- writing frame {} / image {} / upper {}, '{}', delta {} ms\n", logFrameNum, imageFrameNum,
                     upperFrameNum, filename, delta);
        }
      ...
      TLOGD(tlogger, "  -- no imageToExport");
