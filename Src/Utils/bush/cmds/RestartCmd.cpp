...
{
  return std::string("[ roboeireann | robot ]\n")
         + "Restarts roboeireannd or the robot.";
}
...
bool RestartCmd::preExecution(Context& context, const std::vector<std::string>& params)
{
  ...
  else if(params.size() == 0 || params[0] == "roboeireann")
    type = BHUMAND;
...
std::vector<std::string> RestartCmd::complete(const std::string& cmdLine) const
{
  ...
  std::string arguments[ARGS_SIZE] = { "roboeireann", "robot" };
...
bool RestartCmd::RestartTask::execute()
{
  ...
  if(type == BHUMAND)
  {
    std::string command = remoteCommand("systemctl --user restart roboeireann.service", ip);
