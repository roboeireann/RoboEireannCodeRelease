...
void TeamView::init()
{
  if(team)
  {
    ...
    // Note: the order of colours here must match the order of jersey colour constants in RoboCupGameControlData.h
    const char * jerseyColors[] =  { "red", "blue", "yellow", "black", "white", "green", "orange", "purple", "brown", "gray" };
    cbFieldPlayerColor = new QComboBox(this);
    for (const char *jerseyColor : jerseyColors)
      cbFieldPlayerColor->addItem(jerseyColor);

    cbFieldPlayerColor->setCurrentIndex(cbFieldPlayerColor->findText(fromString(team->fieldPlayerColor)));
    cbFieldPlayerColor->setToolTip("Select fieldPlayer color");
    settingsGrid->addWidget(cbFieldPlayerColor);
    connect(cbFieldPlayerColor, SIGNAL(currentIndexChanged(const QString&)), this, SLOT(fieldPlayerColorChanged(const QString&)));


    cbGoalkeeperColor = new QComboBox(this);
    for (const char *jerseyColor : jerseyColors)
      cbGoalkeeperColor->addItem(jerseyColor);
 
    cbGoalkeeperColor->setCurrentIndex(cbGoalkeeperColor->findText(fromString(team->goalkeeperColor)));
    cbGoalkeeperColor->setToolTip("Select goalkeeper color");
    settingsGrid->addWidget(cbGoalkeeperColor);
    connect(cbGoalkeeperColor, SIGNAL(currentIndexChanged(const QString&)), this, SLOT(goalkeeperColorChanged(const QString&)));
...
void TeamView::generateRobotViews(QGridLayout* teamGrid)
{
  ...
  QLabel* substitutes = new QLabel("<b>Substitutes</b> (roboeireann robot software will not be started)");
...
void TeamView::fieldPlayerColorChanged(const QString& color)
{
  if(team)
    team->fieldPlayerColor = toString(color);
}

void TeamView::goalkeeperColorChanged(const QString& color)
{
  if(team)
    team->goalkeeperColor = toString(color);
}
