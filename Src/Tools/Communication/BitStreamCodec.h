...
struct RelativeTimestamp
{
  ...
  RefType refType;
  Integer<unsigned> integer;
  unsigned replaceMaxDiff;

  RelativeTimestamp(const char* name, RefType refType, unsigned maxDiff, unsigned scale = 1, unsigned nbits = 0, unsigned replaceMaxDiff = 0)
      : refType(refType), integer(name, 0, maxDiff, scale, nbits), replaceMaxDiff(replaceMaxDiff)
  {
  }
  ...
  // since relative timestamps usually cannot represent a wide range, we can allow the maxDiff
  // to be replaced with a different value. If it is zero it has no effect. If == max diff
  // it is replaced with replaceMaxDiff
  void read(Reader &r, unsigned &timestamp, unsigned reference) 
  {
    unsigned diff;
    integer.read(r, diff);
    if ((diff == integer.maxVal) && (replaceMaxDiff > integer.maxVal))
      diff = replaceMaxDiff;
