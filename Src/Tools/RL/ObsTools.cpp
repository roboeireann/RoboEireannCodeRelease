...
bool isFacingPoint(float x, float y, float angle) {
    ...
    while (angleBetween > (float)M_PI) {
        angleBetween -= 2 * (float)M_PI;
    }
    while (angleBetween < -(float)M_PI) {
        angleBetween += 2 * (float)M_PI;
    }
    // Check if the angle is within 30 degrees of the target angle
    return std::abs(angleBetween) < (float)M_PI / 6; // was 8 (22.5 degrees)
} 
...
bool isFacingMidfield(float x, float y, float angle) {
    ...
    // Check if the angle is within 30 degrees of the target angle
    return std::abs(angleBetween) < (float)M_PI / 6; // was 2 (90 degrees)
}