 ...
 * @file RLAlg.cpp
 * 
 * This file implements the Algorithm class for RL policies.
 * The Algorithm class constructs, updates and executes the RL policies.
 * 
 * @author BadgerBots
 * 
 */
...
std::vector<float> Algorithm::normalizeObservation(std::vector<float> observation_vector) {
  ...
  for (int i = 0; i < int(observation_vector.size()); i++) {
    float normalized = (observation_vector[i] - normalization_mean[i]) / std::sqrt(normalization_var[i] + normalization_epsilon);
...
void Algorithm::processStdDevAndCov(json::value metadata) {
  ...
  for (auto i = stdArray.begin(); i != stdArray.end(); i++) {
    ...
    float logStdFloat = (std::stof(to_string(logStd)));
    
    std_dev(index) = std::exp(logStdFloat);
    index += 1;
  }
...
std::vector<float> Algorithm::computeCurrentAction(std::vector<NeuralNetwork::TensorXf> action_output,
  ...
  current_log_prob = (float)stats::dmvnorm(action_choice, action_eigen, covariance_matrix, true);

  std::vector<float> current_action = std::vector<float>();
  for (unsigned int i = 0; i < action_choice.size(); i++) {
    current_action.push_back((float)action_choice(i));
  }
