...
DECL_TLOGGER(tlogger, "ModuleGraphRunner", TextLogging::INFO); // switch to DEBUG if you want output
...
void ModuleGraphRunner::update(In& stream)
{
  ...
  // text log the execution order
  if (tlogger.isEnabled(TextLogging::DEBUG))
  {
    TLOGD(tlogger, "Provider execution order:");

    for (const auto& provider : providers)
      TLOGD(tlogger, "  {} update {}", provider.moduleState->module->name, provider.representation);
  }
}
