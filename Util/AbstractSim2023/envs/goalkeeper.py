...
class GoalKeeperEnv(BaseEnv):
    ...
    def move_robot(self, action):
        ...
        self.goalpost_1_distance = np.linalg.norm(np.array([self.robot_x, self.robot_y]) - np.array([-4500,750]))
        self.goalpost_2_distance = np.linalg.norm(np.array([self.robot_x, self.robot_y]) - np.array([-4500,-750]))
    ...
    def calculate_reward(self):
        ...
        if  self.contacted_ball:
            reward += 1
        reward -= (2.0/(self.goalpost_1_distance+.000000001)) #avoid goalposts
        reward -= (2.0/(self.goalpost_2_distance+.000000001)) 
