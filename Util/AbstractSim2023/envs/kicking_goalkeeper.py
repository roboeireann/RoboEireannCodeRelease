...
class KickingGoalKeeperEnv(BaseEnv):
    def __init__(self):
        super().__init__()
    ...
    def move_robot(self, action):
        ...
        if distance_robot_target < (self.robot_radius + self.target_radius) * 5 and self.kicked:
    ...
    def calculate_reward(self):
        reward = 0
        if not self.in_goal_area:
            reward -=.2
        if not self.check_facing_ball():
            reward += -.1
        if self.kicked:
            reward -=.001
            if abs(np.sin(self.relative_angle)) > .34 :
                reward -= 0.1
            if np.cos(self.relative_angle) < 0:
                reward -=0.1
            if  self.contacted_ball:
                reward += 10
        reward -= (1.0/(self.goalpost_1_distance+.000000001)) #avoid goalposts
        reward -= (1.0/(self.goalpost_2_distance+.000000001)) 
...
if __name__ == "__main__":
    ...
    env = VecNormalize.load(
        normalization_path, make_vec_env(KickingGoalKeeperEnv, n_envs=1)
    )
    env.norm_obs = True
