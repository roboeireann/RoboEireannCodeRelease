...
class KickingDefenderEnv(BaseEnv):
    def __init__(self):
        super().__init__()
    ...
    def calculate_reward(self):
        ...
        if self.kicked:
            reward -= .001
            if abs(np.sin(self.relative_angle)) > .34 or np.cos(self.relative_angle) < 0:
                reward -= .01
            if  self.contacted_ball and self.check_facing_ball():
                reward += 1
...
if __name__ == "__main__":
    ...
    env = VecNormalize.load(
        normalization_path, make_vec_env(KickingDefenderEnv, n_envs=1)
    )
    env.norm_obs = True
